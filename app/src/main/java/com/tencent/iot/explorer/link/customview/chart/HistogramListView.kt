...
class HistogramListView : View {
    ...
    fun setMax(max: Int) {
        if (max * histogramWidth > chartWidth) {
            this.max = ((chartWidth - chartWidth % histogramWidth) / histogramWidth).toInt()
            histogramGap = 0f
        } else {
            this.max = max
            if (max <= 0) this.max = 1
            histogramGap = (chartWidth - max * histogramWidth) / (max + 1)
        }
        invalidate()
    }
    ...
    private fun measureMax() {
        val dis = (histogramWidth + histogramGap).toInt()
        max = (chartWidth - chartWidth % dis) / dis
    }
     ...
     * 不可以滑动
     */
    private fun drawMax(canvas: Canvas) {
        list.forEachIndexed { index, value ->
            if (index < max) {
                ...
                val h = adapter.drawHistogram(index,value,chartHeight.toFloat())
                var top = bottom - h
    ...
    inner class HistogramAdapter : Adapter {
        override fun drawY(position: Int): String {
            return "${position * level}"
        }

        override fun drawX(position: Int): String {
            return ""
        }

        override fun drawHistogram(position: Int, value: Long, maxHeight: Float): Float {
    ...
    interface Adapter {
        ...
        fun drawHistogram(position: Int, value: Long, maxHeight: Float): Float
    }
