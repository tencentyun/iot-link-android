    ...
    private fun trtcCallDevice(callingType: Int) {
        HttpRequest.instance.trtcCallDevice("$productId/$deviceName", object: MyCallback {
            ...
            override fun success(response: BaseResponse, reqCode: Int) {
                ...
                val paramJson = json.getJSONObject("TRTCParams")
                val userId = paramJson.getString("UserId")
                val roomId = paramJson.getString("StrRoomId")
                val appId = paramJson.getInteger("SdkAppId")
                val userSig = paramJson.getString("UserSig")

                var room = RoomKey()
                room.userId = userId
                room.appId = appId
                room.userSig = userSig
                room.roomId = roomId
