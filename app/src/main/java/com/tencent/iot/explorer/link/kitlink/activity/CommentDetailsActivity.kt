...
class CommentDetailsActivity: BaseActivity(), View.OnClickListener, MyCallback {
    ...
    private var onDismisListener = object : ShareOptionDialog.OnDismisListener {
        override fun onShareWechatClicked() {
            ...
            Log.e("XXX", "itemJson " + itemJSON!!.toJSONString())
            var path = "pages/Index/TabPages/Evaluation/EvaluationDetail/EvaluationDetail?item={${itemJSON!!.toJSONString()}}"
        ...
        override fun onCopyLinkClicked() {
            AppInfoUtils.copy(this@CommentDetailsActivity, this@CommentDetailsActivity.url2Load)
    ...
    private fun loadContent() {
        ...
        Log.e("XXX", "extraInfo " + extraInfo)
        var tag = "goDetail?"
        if (extraInfo.contains(tag)) {
            var jsonStr = extraInfo.substring(extraInfo.indexOf(tag) + tag.length)
            var json = JSON.parseObject(jsonStr)
            if (json.containsKey(CommonField.KEY_URL) && !TextUtils.isEmpty(json.getString(CommonField.KEY_URL))) {
                comment_detail_web.webViewClient = webViewClient
                comment_detail_web.webChromeClient = webChromeClient

                getAppGetTokenTicket()

                url2Load = CommonField.H5_BASE_URL + "#" + json.getString(CommonField.KEY_URL)
                var itemTag = "item="
                if (json.getString(CommonField.KEY_URL).contains(itemTag)) {
                    var jsonStr = extraInfo.substring(extraInfo.indexOf(itemTag) + itemTag.length, extraInfo.length - 2)
                    jsonStr = jsonStr.replace("\\", "")
                    itemJSON = JSON.parseObject(jsonStr)
                }
            }
        }
    }
