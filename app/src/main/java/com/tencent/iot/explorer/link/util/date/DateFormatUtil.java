...
public class DateFormatUtil {
    ...
    public static String toHMS(long currentTimeMillis) {
        ...
        String s = (ls % 60) + App.Companion.getActivity().getString(R.string.unit_s);
        int im = (int) (ls / 60);
        String m = "";
        if (im > 0) {
            m = (im % 60) + App.Companion.getActivity().getString(R.string.unit_m);//"分钟";
        }
        String h = "";
        int ih = im / 60;
        if (ih > 0) {
            h = ih + App.Companion.getActivity().getString(R.string.unit_h);//"小时";
        }
        return h + m + s;
    }
