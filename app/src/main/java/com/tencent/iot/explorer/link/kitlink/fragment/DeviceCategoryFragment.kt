...
import com.tencent.iot.explorer.link.kitlink.activity.BaseActivity.Companion.REQUEST_BLUETOOTH_PERMISSIONS_CODE
import com.tencent.iot.explorer.link.kitlink.activity.BleConfigHardwareActivity
import com.tencent.iot.explorer.link.kitlink.activity.ProductIntroduceActivity
import com.tencent.iot.explorer.link.kitlink.activity.ScannerActivity
...
import kotlinx.android.synthetic.main.fragment_device_category.container_normal
import kotlinx.android.synthetic.main.fragment_device_category.container_top
import kotlinx.android.synthetic.main.fragment_device_category.gray_line_0
...
import kotlinx.android.synthetic.main.fragment_device_category.vtab_device_category
...
class DeviceCategoryFragment : BaseFragment(), MyCallback, CRecyclerView.RecyclerItemView,
    View.OnClickListener, VerticalTabLayout.OnTabSelectedListener {
    ...
    private var toMainAction: (() -> Unit)? = null

    private val fromSource: Int by lazy {
        arguments?.getInt("formSourceType") ?: FormSourceType.FROM_MAIN_ACTIVITY
    }
    ...
    fun setToMainListener(action: () -> Unit) {
        this.toMainAction = action
    }
    ...
    override fun startHere(view: View) {
        iv_back.isVisible = fromSource != FormSourceType.FROM_MAIN_ACTIVITY
        ...
        HttpRequest.instance.deviceProducts(productsList, object : MyCallback {
            ...
            override fun success(response: BaseResponse, reqCode: Int) {
                if (response.isSuccess()) {
                    ...
                    var products =
                        JSON.parseObject(response.data.toString(), ProductsEntity::class.java)
    ...
    private var bleDeviceConnectionListener = object : BleDeviceConnectionListener {
        ...
        override fun onBleReportActionResult(
            reason: Int,
            actionId: Int,
            bleDeviceProperty: BleDeviceProperty
        ) {
        }
    ...
    private var onBleDevSeclectedListener = object : BleDeviceAdapter.OnItemClicked {
        override fun onItemClicked(pos: Int, dev: BleDevice) {
            ...
            BleConfigHardwareActivity.startWithProductid(requireContext(), dev.productId, dev.type)
            BleConfigService.get().stopScanBluetoothDevices()
        }
    }
        ...
        when (reqCode) {
            RequestCode.get_parent_category_list -> {
                if (response.isSuccess()) {
                    response.parse(DeviceCategoryListResponse::class.java)?.run {
                        ...
                        val adapter = MyTabAdapter(requireContext())
                        for (item in List) {
            ...
            RequestCode.scan_bind_device, RequestCode.sig_bind_device -> {
                if (response.isSuccess()) {
                    ...
                    if (fromSource == FormSourceType.FROM_MAIN_ACTIVITY){
                        toMainAction?.invoke()
                    }else{
                        requireActivity().finish()
                    }
                    val resData =
                        JSON.parseObject(response.data.toString(), BindDevResponse::class.java)
                    bindSubDev(resData)
                } else {
                    T.show(response.msg)
                }
            ...
            RequestCode.bind_gateway_sub_device -> {
                if (response.isSuccess()) {
                    ...
                    App.data.setRefreshLevel(2)
                    com.tencent.iot.explorer.link.kitlink.util.Utils.sendRefreshBroadcast(
                        requireContext()
                    )
                } else {
                    T.show(response.msg)
    ...
    private fun bindSubDev(gatwayDev: BindDevResponse) {
        if (gatwayDev != null && gatwayDev.Data != null && gatwayDev.Data!!.AppDeviceInfo != null) {
            ...
            if (dev.DeviceType == "1") {
                HttpRequest.instance.gatwaySubDevList(
                    dev.ProductId,
                    dev.DeviceName,
                    object : MyCallback {
                        override fun fail(msg: String?, reqCode: Int) {
                        ...
                        override fun success(response: BaseResponse, reqCode: Int) {
                            if (response.isSuccess()) {
                                var gatewaySubDevsResp = JSON.parseObject(
                                    response.data.toString(),
                                    GatewaySubDevsResp::class.java
                                )
                                if (gatewaySubDevsResp != null && gatewaySubDevsResp.DeviceList != null && gatewaySubDevsResp.DeviceList.size > 0) {
                                    for (subDev in gatewaySubDevsResp.DeviceList) {
                                        if (subDev.BindStatus == 0) {
                                            HttpRequest.instance.bindGatwaySubDev(
                                                dev.ProductId,
                                                dev.DeviceName,
                                                subDev.ProductId,
                                                subDev.DeviceName,
                                                this@DeviceCategoryFragment
                                            )
                                        }
                                    }
                                }
    ...
    override fun onRequestPermissionsResult(
        ...
        if (requestCode == BaseActivity.DEFAULT_REQUEST_PERMISSIONS_CODE || requestCode == BaseActivity.REQUEST_BLUETOOTH_PERMISSIONS_CODE) {
        ...
        if (requestCode == BaseActivity.DEFAULT_REQUEST_PERMISSIONS_CODE) {
            ...
            //同意了相机权限
            var intent = Intent(Intent(requireContext(), ScannerActivity::class.java))
    ...
    protected fun requestPermission(
        permissions: Array<String>,
        requestCode: Int = BaseActivity.DEFAULT_REQUEST_PERMISSIONS_CODE
    ) {
    ...
    protected fun checkPermissions(permissions: Array<String>): Boolean {
        for (p in permissions) {
            if (ActivityCompat.checkSelfPermission(requireContext(), p) == PackageManager.PERMISSION_DENIED) {
                L.e(p + "被拒绝")
                return false
            }
            L.e(p + "已经申请成功")
        ...
        when (v) {
            iv_scann -> {
                if (checkPermissions(permissions)) {
                    ...
                    var cameraJsonString = Utils.getStringValueFromXml(
                        T.getContext(),
                        CommonField.PERMISSION_CAMERA,
                        CommonField.PERMISSION_CAMERA
                    )
                    ...
                    permissionDialog = PermissionDialog(
                        App.activity,
                        R.mipmap.permission_camera,
                        getString(R.string.permission_camera_lips),
...
//            }
                    ...
                    when {
                        ...
                        contains("page=adddevice") && contains("productId") -> {
                            ...
                            if (contains("preview=1")) {
                                var intent =
                                    Intent(requireContext(), ProductIntroduceActivity::class.java)
        ...
        override fun success(response: BaseResponse, reqCode: Int) {
            if (response.isSuccess() && reqCode == RequestCode.get_products_config) {
                response.parse(ProductsConfigResponse::class.java)?.run {
                    ...
                    if (config != null && !TextUtils.isEmpty(config.Global) && ProductGlobal.isProductGlobalLegal(
                            config.Global
                        )
                    ) {
                        var intent = Intent(requireContext(), ProductIntroduceActivity::class.java)
                        intent.putExtra(CommonField.PRODUCT_ID, productId)
                        startActivity(intent)
                        return@run
                    }
                    ...
                    if (wifiConfigTypeList.equals("{}") || TextUtils.isEmpty(wifiConfigTypeList)) {
                        SmartConfigStepActivity.startActivityWithExtra(requireContext(), productId)
                        ...
                        if (typeList.size > 0 && typeList[0] == "softap") {
                            SoftApStepActivity.startActivityWithExtra(requireContext(), productId)
                        } else if (typeList.size > 0 && typeList[0] == "llsyncble") {
                            BleConfigHardwareActivity.startWithProductid(
                                requireContext(),
                                productId
                            )
                        } else {
                            SmartConfigStepActivity.startActivityWithExtra(
                                requireContext(),
                                productId
                            )
                        }
    ...
    private fun bindDevice(signature: String) {
        HttpRequest.instance.scanBindDevice(
    ...
    private fun bleSigBindDevice(deviceInfo: TrtcDeviceInfo, bindType: String) {
        HttpRequest.instance.sigBindDevice(
            App.data.getCurrentFamily().FamilyId, App.data.getCurrentRoom().RoomId,
    ...
    private fun beginScanning() {
        ...
        if (!hasPermission) {
            ...
            permissionDialog = PermissionDialog(
                ...
                getString(R.string.permission_location_ssid_ble)
            )
            permissionDialog!!.show()
        ...
        val rotateAnimation: Animation = AnimationUtils.loadAnimation(requireContext(), R.anim.circle_rotate)
    ...
    override fun onTabSelected(tab: TabView?, position: Int) {
        ...
        for (i in 0 until App.data.numOfCategories) {
            if (i != position)
                vtab_device_category.getTabAt(i)
                    .setBackgroundColor(resources.getColor(R.color.gray_F5F5F5))
        }
