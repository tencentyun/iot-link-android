        ...
        if (action == MessageConst.DEVICE_CHANGE) { //收到了设备属性改变的wss消息
            ...
            if (subType == MessageConst.REPORT) { //收到了设备端属性状态改变的wss消息
                ...
                if (App.data.callingDeviceId != "" && !deviceId.equals(userId)) {
                    val currentActivityName = activity?.localClassName ?:""
                    if (currentActivityName.contains("TRTCAudioCallActivity") || currentActivityName.contains("TRTCVideoCallActivity")) { //当前正显示音视频通话页面，finish掉
                        activity?.finish()
                    }
