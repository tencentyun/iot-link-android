...
class App : Application(), Application.ActivityLifecycleCallbacks, PayloadMessageCallback,
    ...
    override fun payloadMessage(payload: Payload) {
        ...
        TRTCUIManager.getInstance().payloadMessage(trtcPayload, userId, object: TestCallback {
            override fun busy() {
            ...
            override fun updateCallStatus(k: String?, v: String?, deviceId: String?) {
                controlDevice(k!!, v!!, deviceId!!)
            }
...
//                if (!rejectId.isEmpty()) {
//                    if (TRTCUIManager.getInstance().callingDeviceId != "" && rejectId == userId && TRTCUIManager.getInstance().isCalling) {
//                        TRTCUIManager.getInstance().userBusy()
//                        TRTCUIManager.getInstance().exitRoom()
//                        activity?.runOnUiThread {
//                            Toast.makeText(activity, "对方正忙...", Toast.LENGTH_LONG).show()
...
//                if (TRTCUIManager.getInstance().callingDeviceId != "" && deviceId != userId) {
//                    if (TRTCUIManager.getInstance().isCalling) { //当前正显示音视频通话页面，finish掉
//                        if (TRTCUIManager.getInstance().callingDeviceId == payload.deviceId) {
//                            TRTCUIManager.getInstance().userBusy()
//                            TRTCUIManager.getInstance().exitRoom()
//                            activity?.runOnUiThread {
//                                Toast.makeText(activity, "对方正忙...", Toast.LENGTH_LONG).show()
...
//                if (TRTCUIManager.getInstance().callingDeviceId == "" && TRTCUIManager.getInstance().isCalling) {
//                    if (videoCallStatus == TRTCCallStatus.TYPE_CALLING.value) {
//                        controlDevice(MessageConst.TRTC_VIDEO_CALL_STATUS, "0", payload.deviceId)
//                    } else if (audioCallStatus == TRTCCallStatus.TYPE_CALLING.value) {
//                        controlDevice(MessageConst.TRTC_AUDIO_CALL_STATUS, "0", payload.deviceId)
...
//                if (videoCallStatus == 1) {
//                    if (TRTCUIManager.getInstance().callingDeviceId == "" && deviceId != "" && !deviceId.contains(userId)) { //App被动呼叫 _sys_userid有值 且不包含当前用户的userid
//                    } else {
...
//                    if (TRTCUIManager.getInstance().callStatus == TRTCCallStatus.TYPE_CALLING.value && TRTCUIManager.getInstance().callingDeviceId == "") {
//                        activity?.runOnUiThread {
//                            Toast.makeText(activity, "其他用户已接听...", Toast.LENGTH_LONG).show()
//                        }
//                        TRTCUIManager.getInstance().otherUserAccept()
