...
class App : Application(), Application.ActivityLifecycleCallbacks, StartBeingCallCallback {
    ...
    fun requestDeviceList() {
        HttpRequest.instance.deviceList(
            SharePreferenceUtil.getString(activity, App.CONFIG, CommonField.CURRENT_FAMILY_ID),
            SharePreferenceUtil.getString(activity, App.CONFIG, CommonField.CURRENT_ROOM_ID),
            0,
            object: MyCallback {
                ...
                object : MyCallback {
                    ...
                    override fun success(response: BaseResponse, reqCode: Int) {
                        if (response.code == 0) { //获取 设备当前状态(如亮度、开关状态等) 成功
                            ...
                            val videoCallStatusJson =
                                dataJson.getJSONObject(MessageConst.TRTC_VIDEO_CALL_STATUS)
