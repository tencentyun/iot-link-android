...
class DelayTimeActivity : BaseActivity() {
    ...
    private fun initDatePicker() {
        ...
        if (editExtra.editAble) {
            handler.postDelayed( {
                wheel_delay_time_hour.setSelectedItemPosition(editExtra.currentHour, true) },
                100)
            handler.postDelayed( {
                wheel_delay_time_min.setSelectedItemPosition(editExtra.currentMinute, true) },
                100)
        }
