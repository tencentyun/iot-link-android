...
public class TRTCVideoCallActivity extends AppCompatActivity implements NetWorkStateReceiver.NetworkStateReceiverListener {
    ...
    private void checkAndRequestPermission() {
        ...
        if (ActivityCompat.checkSelfPermission(this, Manifest.permission.WRITE_EXTERNAL_STORAGE) == PackageManager.PERMISSION_DENIED ||
            ...
            String storageJsonString = Utils.INSTANCE.getStringValueFromXml(this, CommonField.PERMISSION_STORAGE, CommonField.PERMISSION_STORAGE);
            long lasttime = 0L;
            if (storageJsonString != null) {
                JSONObject storageJson = JSON.parseObject(storageJsonString);
                lasttime = storageJson.getLong(CommonField.PERMISSION_STORAGE);
            }
            if (storageJsonString != null && lasttime > 0 && System.currentTimeMillis() / 1000 - lasttime < 48 * 60 * 60) {
                T.show(getString(com.tencent.iot.explorer.link.R.string.permission_of_camera_mic_refuse));
                return;
            }
            if (permissionDialog == null) {
                permissionDialog = new PermissionDialog(this, com.tencent.iot.explorer.link.R.mipmap.permission_album ,getString(com.tencent.iot.explorer.link.R.string.permission_storage_lips), getString(com.tencent.iot.explorer.link.R.string.permission_storage));
                permissionDialog.show();
                requestStoragePermission = true;
            }
            ActivityCompat.requestPermissions(this, new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE, Manifest.permission.READ_EXTERNAL_STORAGE}, 104);

            // 记录请求storage权限的时间
            JSONObject json = new JSONObject();
            json.put(CommonField.PERMISSION_STORAGE, System.currentTimeMillis() / 1000);
            Utils.INSTANCE.setXmlStringValue(this, CommonField.PERMISSION_STORAGE, CommonField.PERMISSION_STORAGE, json.toJSONString());
            return;
        }
        boolean calling = initData();
        initListener();
