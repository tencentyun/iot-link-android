    ...
    private fun parseRspMessage(reqId: Int, message: String) {
        ...
        try {
            JSON.parseObject(message, ResponseMessage::class.java)?.run {
                data?.let {
                    if (it.Response == null) {
                        failMessage.setErrorMessage(App.activity!!.getString(R.string.paser_data_error_resp_null)) //"解析响应数据时，Response为空"
                    ...
                    if (it.Response!!.getJSONObject(MessageConst.ERROR) == null) {
                        ...
                        try {
                            ...
                            e.printStackTrace()
                            failMessage.setErrorMessage(App.activity!!.getString(R.string.paser_data_error_with_respfailmessage))
                        }
                        dispatchCallback?.yunMessageFail(reqId, message, failMessage)
                    }
                }
            ...
            e.printStackTrace()
            failMessage.setErrorMessage(App.activity!!.getString(R.string.paser_data_error)) //解析响应数据时发生错误
            dispatchCallback?.yunMessageFail(reqId, message, failMessage)
        }
    }
