...
class HttpRequest private constructor() {
    ...
    private fun postJson(param: HashMap<String, Any>, callback: MyCallback, reqCode: Int) {
        ...
        if (!App.isOEMApp()) {// 公版&开源版
            ...
            api = if (App.DEBUG_VERSION) "$OPENSOURCE_APP_API/$action" + "?uin=archurtest" else "$OPENSOURCE_APP_API/$action" + "?uin=${App.data.deviceId}"
        } else {// OEM版
    ...
    private fun tokenPost(param: HashMap<String, Any>, callback: MyCallback, reqCode: Int) {
        ...
        val api:String
        if (!App.isOEMApp()) {// 公版&开源版
            api = if (App.DEBUG_VERSION) OPENSOURCE_TOKEN_API + "?uin=archurtest" else OPENSOURCE_TOKEN_API + "?uin=${App.data.deviceId}"
        } else {// OEM版
            api = OEM_TOKEN_API
        }
        StringRequest.instance.postJson(api, json, object : Callback {
    ...
    private fun getH5tokenPost(param: HashMap<String, Any>, callback: MyCallback, reqCode: Int) {
        ...
        if (!App.isOEMApp()) {// 公版&开源版
            ...
            api = if (App.DEBUG_VERSION) OEM_TOKEN_API + "?uin=archurtest" else OEM_TOKEN_API + "?uin=${App.data.deviceId}"
        }
