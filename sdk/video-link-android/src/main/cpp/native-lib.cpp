...
static int flv_onwrite(void *param, const struct flv_vec_t* vec, int n) {
    ...
    if (listener == nullptr) {
        LOGE("please set muxer listener first!");
        return 0;
    }
...
Java_com_tencent_iot_video_link_util_audio_FLVPacker_init(JNIEnv *env, jobject thiz) {
    writer = flv_writer_create2(1, 0, flv_onwrite, nullptr);
    muxer = flv_muxer_create(flv_onmuxer, writer);
    return JNI_OK;
}
...
extern "C"
JNIEXPORT void JNICALL
Java_com_tencent_iot_video_link_util_audio_FLVPacker_release(JNIEnv *env, jobject thiz) {
