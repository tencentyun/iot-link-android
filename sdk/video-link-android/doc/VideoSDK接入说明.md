...
* 下载路径
  ...
  SNAPSHOT版：
    ...
    * (2) 路径：Repositories --> Snapshots --> 在path look up
      输入框中输入com/tencent/iot/thirdparty/android --> xp2p-sdk -->版本号(例如：1.0.0-SNAPSHOT) -->
      选择最新的aar右键下载
...
setUserCallbackToXp2p(_av_data_recv, _msg_notify);
/* 初始化p2p */
startService(this,product_id,device_name,xp2p_info,xP2PAppConfig);
```
...
| timeout_us | long   | 命令超时时间，单位为微秒，值为0时采用默认超时(7500ms左右) | 输出    |
...
| xp2PAppConfig | XP2PAppConfig | 请求log以及配置的config参数 | 输入    |
...
第三方App在接入Video SDK时，建议将`secretId`和`secretKey`保存到自建后台，不推荐将这两个信息保存至App端;
而SDK需要的xp2p info需要App侧从自己的业务后台获取；获取到xp2p info后，可以通过上述的`startService`
接口将该info传给SDK，示例代码如下：
