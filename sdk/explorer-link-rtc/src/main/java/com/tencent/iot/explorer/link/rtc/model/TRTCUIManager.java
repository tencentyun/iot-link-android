...
public class TRTCUIManager {
    ...
    public void payloadMessage(TRTCPayload trtcPayload, String userId, TestCallback callback) {
        ...
        try {
            ...
            if (Common.TRTC_DEVICE_CHANGE.equals(action)) { //收到了设备属性改变的wss消息
                ...
                if (Common.TRTC_REPORT.equals(subType)) { //收到了设备端上报的属性状态改变的wss消息
                    ...
                    String rejectId = "";
                    if (payloadParamsJson.has(Common.TRTC_TRTC_EXTRA_INFO)) {
                        JSONObject extraInfo = new JSONObject(payloadParamsJson.getString(Common.TRTC_TRTC_EXTRA_INFO));
                        if (extraInfo.has(Common.TRTC_TRTC_REJECT_USERID)) {
                            rejectId = extraInfo.getString(Common.TRTC_TRTC_REJECT_USERID);
                        }
                    }
                    ...
                    if (!rejectId.isEmpty()) {
                        if (!TextUtils.isEmpty(callingDeviceId) && rejectId.equals(userId) && TRTCUIManager.getInstance().isCalling) {
                            callback.busy();
                        }
                        return;
                    }
                    ...
                    if (TextUtils.isEmpty(callingDeviceId) && TRTCUIManager.getInstance().isCalling) {
                        if (videoCallStatus == TRTCCallStatus.TYPE_CALLING.getValue()) {
                            callback.updateCallStatus(Common.TRTC_VIDEO_CALL_STATUS, "0", payloadDeviceId);
                        } else if (audioCallStatus == TRTCCallStatus.TYPE_CALLING.getValue()) {
                            callback.updateCallStatus(Common.TRTC_AUDIO_CALL_STATUS, "0", payloadDeviceId);
                    ...
                    if (videoCallStatus == 1) {
                        ...
                        if (TRTCUIManager.getInstance().deviceId == payloadDeviceId) {
                            callback.hungUp();
                        }
                    } else if (videoCallStatus == 2 || audioCallStatus == 2) {
                        if (TRTCUIManager.getInstance().callStatus == TRTCCallStatus.TYPE_CALLING.getValue() && callingDeviceId == "") {
