        ...
        scanCallback = object : ScanCallback() {
            override fun onBatchScanResults(results: List<ScanResult?>?) {
                        ...
                        result?.scanRecord?.serviceUuids?.let {
                            for (value in it) {
                                ...
                                if (value.uuid.toString().substring(4, 8).toUpperCase().equals("FFF0")) {
                                    ...
                                    dev?.blueDev = result?.device
                                    dev?.type = 1
                                    Log.e("XXX", "boundState " + dev?.boundState)
                                    break
                                }
                            }
                        }
                ...
                dev?.let { bleDev ->
                    ...
                    Log.e("XXX", "founded " + JSON.toJSONString(founded))
                ...
                override fun onCharacteristicChanged(gatt: BluetoothGatt?, characteristic: BluetoothGattCharacteristic?) {
                    ...
                    characteristic?.let {
                        if (it.uuid.toString().substring(4, 8).toUpperCase().equals("FFE3")) {
                            ...
                            Log.e("XXX", "characteristic ${bytesToHex(it.value)}")
                            when (it.value[0]) {
                                0x08.toByte() -> {
                                    L.d(TAG, "0x08")
                                    // 测试使用
...
//    }
    ...
    fun connectSubDevice(connection: BluetoothGatt?): Boolean {
        ...
        Log.e("XXX", "signData ${bytesToHex(signData)}")
        Log.e("XXX", "signData size: ${signData.size}")
        System.arraycopy(signData, 0, byteArr, 7, 20)
        Log.e("XXX", "connectSubDevice byteArr size: ${bytesToHex(byteArr)}")
        return setFFE0CharacteristicValue(connection, byteArr)
    }
    ...
    fun unbind(connection: BluetoothGatt?): Boolean  {
        ...
        Log.e("XXX", "unbind " + bytesToHex(localPsk))
