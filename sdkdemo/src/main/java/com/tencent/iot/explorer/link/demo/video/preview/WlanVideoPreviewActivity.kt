    ...
    override fun chgTextState(value: Int) {
        ...
        when(value) {
            0 -> {
                ...
                command = "ipc.flv?action=live&_protocol=tcp&quality=super&_crypto=off&_port=$port"
            }
            1 -> {
                tv_video_quality.setText(R.string.video_quality_medium_str)
                command = "ipc.flv?action=live&_protocol=tcp&quality=high&_crypto=off&_port=$port"
            }
            2 -> {
                tv_video_quality.setText(R.string.video_quality_low_str)
                command = "ipc.flv?action=live&_protocol=tcp&quality=standard&_crypto=off&_port=$port"
            }
        }
    ...
    override fun speakAble(able: Boolean): Boolean {
        App.data.accessInfo?.let { accessInfo ->
            ...
            if (able) {
                ...
                XP2P.runSendService(id, command, false)
