...
class VideoLocalPlaybackFragment: VideoPlaybackBaseFragment(), TextureView.SurfaceTextureListener, XP2PCallback, CoroutineScope by MainScope() {
    ...
    @Volatile
    private var currentPlayerState = false
    ...
    private fun startJobRereshTimeAndProgress() {
        seekBarJob = launch {
            while (isActive) {
                ...
                Log.e("XXX", "player.currentPosition ${player.currentPosition}")
            }
        }
    }
    ...
    private fun playVideo(startTime: Long, endTime: Long, offset: Long) {
        Log.e("XXX", "startTime $startTime")
        Log.e("XXX", "endTime $endTime")
    ...
    private fun setPlayerUrl(suffix: String, offset: Long) {
        ...
        player?.let {
            ...
            Log.e("XXX", "setPlayerUrl url ${url}")
            it.reset()
            ...
            if (offset > 0) {
                devInfo?.let { dev ->
                    ...
                    XP2P.postCommandRequestSync(id, seekCommand.toByteArray(), seekCommand.toByteArray().size.toLong(), 2 * 1000 * 1000)
                }
            }
    ...
    override fun xp2pEventNotify(id: String?, msg: String?, event: Int) {
        ...
        if (event == 1003) {
            ...
            currentPostion = player.currentPosition
            Log.e("XXX", "currentPostion ${currentPostion}")
            launch (Dispatchers.Main) {
