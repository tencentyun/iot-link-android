...
class VideoLocalPlaybackFragment : VideoPlaybackBaseFragment(), TextureView.SurfaceTextureListener,
        ...
        object : LocalPlaybackListAdapter.OnDownloadClickedListener {
            override fun onItemDownloadClicked(pos: Int) {
                ...
                XP2P.startAvRecvService(
                    ...
                    "action=download&channel=0&file_name=${playbacks[pos].file_name}&offset=0",
                    false
                )
                initDownload(playbacks[pos].file_name)
            }
        }
    ...
    private fun refreshDateTime(date: Date) {
        ...
        playbacks.clear()
        playbacks.addAll(currentPlaybacks)
    ...
    private fun keepConnect(id: String?) {
        ...
        Thread {
            ...
            while (true) {
                ...
                Log.d(
                    TAG,
                    "id=${id} keepConnect do not wait and keepAliveThreadRuning=${keepAliveThreadRuning}"
                )
...
//                Log.d(TAG, "id=${id}, tmpCountDownLatch do not wait any more")
    ...
    private fun initDownload(fileName: String) {
        ...
        if (Environment.getExternalStorageState() == Environment.MEDIA_MOUNTED) { // 优先保存到SD卡中
            path = Environment.getExternalStorageDirectory().absolutePath + File.separator + "local_playback"
        } else { // 如果SD卡不存在，就保存到本应用的目录下
            path = context?.filesDir?.absolutePath + File.separator + "local_playback"
        }
        val file = File(path)
        if (!file.exists()) {
            if (file.mkdirs()) {
                L.e("创建本地回放存储路径成功")
                val videoName = path + File.separator + fileName
