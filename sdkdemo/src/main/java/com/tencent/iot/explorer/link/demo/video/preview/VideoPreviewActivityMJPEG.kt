...
open class VideoPreviewActivityMJPEG : VideoBaseActivity(), EventView, TextureView.SurfaceTextureListener,
    XP2PCallback, CoroutineScope by MainScope(), VolumeChangeObserver.VolumeChangeListener {

    open var tag = VideoPreviewActivityMJPEG::class.simpleName
    ...
    override fun initView() {
        presenter = EventPresenter(this@VideoPreviewActivityMJPEG)
        ...
        var linearLayoutManager = LinearLayoutManager(this@VideoPreviewActivityMJPEG)
        adapter = ActionListAdapter(this@VideoPreviewActivityMJPEG, records)
        ...
        today_tip.setText(getString(R.string.today) + " " + CommonUtils.getWeekDay(this@VideoPreviewActivityMJPEG))
        records.clear()
        ...
        Thread(Runnable {
            ...
            if (started != 0) {
                launch(Dispatchers.Main) {
                    ...
                    Toast.makeText(this@VideoPreviewActivityMJPEG, errInfo, Toast.LENGTH_SHORT).show()
                }
                return@Runnable
            }
...
//                Log.d(tag, "id=${id}, tmpCountDownLatch do not wait any more")
        ...
        App.data.accessInfo?.let { accessInfo ->
            if (able) {
                ...
                launch(Dispatchers.Main) {
                    ...
                    Toast.makeText(this@VideoPreviewActivityMJPEG, retContent, Toast.LENGTH_SHORT).show()
                }
    ...
    override fun setListener() {
        ...
        radio_record.setOnCheckedChangeListener { buttonView, isChecked ->
            if (isChecked) {
                ...
                player.stopRecord()
                CommonUtils.refreshVideoList(this@VideoPreviewActivityMJPEG, filePath)
            }
        }
        radio_playback.setOnClickListener {
            var dev = DevInfo()
            dev.deviceName = presenter.getDeviceName()
            VideoPlaybackActivity.startPlaybackActivity(this@VideoPreviewActivityMJPEG, dev)
        }
        radio_photo.setOnClickListener {
            ImageSelect.saveBitmap(this@VideoPreviewActivityMJPEG, v_preview.bitmap)
    ...
    private fun showVVideoQualityDialog() {
        ...
        var dlg = ListOptionsDialog(this@VideoPreviewActivityMJPEG, options)
        dlg.show()
    ...
    private fun showHVideoQualityDialog() {
        ...
        var dlg = VideoQualityDialog(this@VideoPreviewActivityMJPEG, pos)
        dlg.show()
    ...
    override fun xp2pEventNotify(id: String?, msg: String?, event: Int) {
        ...
        if (event == 1003) {
            ...
            launch(Dispatchers.Main) {
                ...
                Toast.makeText(this@VideoPreviewActivityMJPEG, content, Toast.LENGTH_SHORT).show()
            }
    ...
    companion object {
        fun startPreviewActivity(context: Context?, dev: DevInfo) {
            ...
            var intent = Intent(context, VideoPreviewActivityMJPEG::class.java)
            var bundle = Bundle()
